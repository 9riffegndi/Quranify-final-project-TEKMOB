...
/// Service to manage level completion and progress tracking for Sambung Ayat game
class SambungAyatProgressService {
  static const String _sambungAyatLevelPrefix = 'sambung_ayat_level_';
  static const String _sambungAyatStarsPrefix = 'sambung_ayat_stars_';
  ...
  static Future<bool> isLevelUnlocked(int level) async {
    ...
    return prefs.getBool('${_sambungAyatLevelPrefix}${level - 1}') ?? false;
  }
  ...
  static Future<List<int>> getUnlockedLevels(int maxLevels) async {
    ...
    for (int i = 1; i < maxLevels; i++) {
      final isUnlocked =
          prefs.getBool('${_sambungAyatLevelPrefix}${i - 1}') ?? false;
      if (isUnlocked) {
  ...
  static Future<void> completeLevel(int level, int stars) async {
    ...
    await prefs.setBool('${_sambungAyatLevelPrefix}$level', true);

    // Update stars if it's a higher score
    final currentStars = prefs.getInt('${_sambungAyatStarsPrefix}$level') ?? 0;
    if (stars > currentStars) {
      await prefs.setInt('${_sambungAyatStarsPrefix}$level', stars);
    }
  }
  ...
  static Future<int> getLevelStars(int level) async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt('${_sambungAyatStarsPrefix}$level') ?? 0;
  }
  ...
  static Future<int> getTotalStars(int maxLevels) async {
    ...
    for (int i = 0; i < maxLevels; i++) {
      totalStars += prefs.getInt('${_sambungAyatStarsPrefix}$i') ?? 0;
    }
  ...
  static Future<void> resetAllProgress() async {
    ...
    final levelKeys = keys.where(
      (key) =>
          key.startsWith(_sambungAyatLevelPrefix) ||
          key.startsWith(_sambungAyatStarsPrefix),
    );
